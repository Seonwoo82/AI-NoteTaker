...
회의록의 **AI 보완**에 이름 오류나 빠진 맥락을 입력합니다. 예를 들어 “승현은 승원이며, 제안서 전달 일정은 내일이 아니라 다음 주입니다”라고 요청할 수 있습니다. **보완안 생성 → 미리보기 → 적용** 순서로 반영하고, 버리기·취소·실패 시 기존 회의록은 유지합니다. 보완 과정에서 원래 전사문은 바꾸지 않습니다.
...
- Mac/iOS 1.4.1 빌드 13을 두 기기에 설치하고 Mac 실행을 확인했습니다. 실제 iPhone 목소리 등록은 사용자 확인을 기다립니다. 개인 목소리 등록 데이터는 전송하거나 대신 녹음하지 않았습니다. 자세한 내용은 [목소리 등록 검증 기록](docs/implementation/VOICE-ENROLLMENT-GUIDE.md)에 있습니다.

## 검증 및 배포 현황

현재 문서가 반영하는 버전은 **Mac/iOS 1.4.1, 빌드 13**입니다.

| 항목 | 상태 | 결과 |
| --- | --- | --- |
| Mac 전체 테스트 | 통과 | 400개 통과 |
| iOS 시뮬레이터 전체 테스트 | 통과 | 276개 통과 |
...
| 1.4.1 빌드 13 Mac Release | 완료 | Release 빌드 통과, 코드서명 검증, `/Applications/AI-NoteTaker.app` 설치 및 실행 확인 |
| 1.4.1 빌드 13 iOS Release | 완료 | Release 빌드 통과, IPA `build/releases/AI-NoteTaker-iOS-1.4.1-build13.ipa` 생성과 버전 검증, 실제 iPhone 설치 완료 |
| Cloudflare Worker 배포 | 완료 | 참여자 전사·보완 모델 동기화 배포, 인증된 API 조회 200 |
| iPhone 모델 파일 | 완료 | 공개 모델 파일 13개가 1.3.2 업데이트 후 보존됨을 확인 |
| 목소리 등록 안정성 | 통과 | 오디오 콜백, 연속 입력 시간 오차, 취소·재등록 회귀 테스트 통과 |
| iPhone 설치 | 완료 | 1.4.1 빌드 13 설치 완료 |
