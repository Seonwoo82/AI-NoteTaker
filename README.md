...
2026-09-08 기준 AudioPipeline 테스트 **235개**, 앱 단위 테스트 **193개**, AI 설정·자동 생성·Markdown 복사 UI 테스트 **2개**를 통과했습니다. 실제 AVAudioEngine으로 합성 음원을 렌더링해 무음이 아닌 출력도 확인했습니다. 메뉴 막대/창 재열기와 실제 Canvas 파형이 전환 후에도 그려지는지 검사하는 UI 회귀 테스트도 포함합니다. 실제 OpenRouter 공개 모델 목록 파싱을 확인했으며, 유료 AI 요청은 테스트용 응답으로 검증했습니다. 실제 전사·회의록 품질과 모델별 비용은 사용자가 설정한 API 키와 선택한 제공자에서 확인해야 합니다.
