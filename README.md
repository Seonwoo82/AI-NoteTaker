...
2026-09-08 기준 AudioPipeline 테스트 **235개**, 앱 단위 테스트 **206개**, AI 설정·자동 생성·Markdown 복사 UI 테스트 **2개**를 통과했습니다. 실제 AVAudioEngine으로 합성 음원을 렌더링해 무음이 아닌 출력도 확인했습니다. 메뉴 막대/창 재열기, 실제 Canvas 파형 복구, 목록 인라인 이름 변경과 바깥 클릭 저장 및 Esc 취소를 검사하는 UI 회귀 테스트도 포함합니다. AI 회귀 테스트는 합성 응답을 사용합니다. GLM-5.3의 실제 API에서 추론으로 출력 한도가 소진된 응답을 별도로 재현했으며, 수정 후 전사 캐시 재사용부터 회의록 생성·저장·앱 표시까지 확인했습니다. 실제 녹음·전사문·회의록은 저장소에 포함하지 않습니다. 모델별 품질과 비용은 입력 내용 및 선택한 제공자에 따라 달라집니다.
