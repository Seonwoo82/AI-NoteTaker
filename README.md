...
현재 버전은 **1.5.1 (빌드 15)**입니다. Mac 폴더 펼침·접기, 녹음 드래그 이동과 폴더 순서 변경을 지원하며, 전체 목록 복귀와 iPhone 폴더 탐색을 보완했습니다. [변경 및 검증 기록](docs/implementation/FOLDER-NAVIGATION-AND-DRAG.md)
...
- Mac/iOS 1.5.1 빌드 15를 두 기기에 설치하고 Mac 실행을 확인했습니다. iPhone 실행 확인은 기기 잠금 해제 후 진행합니다. 실제 iPhone 목소리 등록은 사용자 확인을 기다립니다. 개인 목소리 등록 데이터는 전송하거나 대신 녹음하지 않았습니다. 자세한 내용은 [목소리 등록 검증 기록](docs/implementation/VOICE-ENROLLMENT-GUIDE.md)에 있습니다.

## 검증 및 배포 현황

현재 문서가 반영하는 버전은 **Mac/iOS 1.5.1, 빌드 15**입니다.

| 항목 | 상태 | 결과 |
| --- | --- | --- |
| Mac 전체 테스트 | 통과 | 434개 통과 |
| iOS 시뮬레이터 전체 테스트 | 통과 | 303개 통과 |
| Cloudflare Worker 테스트 | 통과 | 57개 통과 |
...
| 1.5.1 빌드 15 Mac Release | 완료 | AI 작업 완료 후 백업·설치, 코드서명·실행·실제 폴더 펼침 및 전체 목록 복귀 확인 |
| 1.5.1 빌드 15 iOS Release | 설치 완료 | Release 빌드·코드서명·로컬 IPA·실제 기기 버전 검증, 기기 잠금으로 실행 확인 대기 |
| Cloudflare Worker 배포 | 완료 | 폴더 순서 필드와 구버전 호환 처리 배포, 기존 데이터·폴더 API 인증 조회 200 |
| iPhone 모델 파일 | 완료 | 공개 모델 파일 13개가 1.3.2 업데이트 후 보존됨을 확인 |
| 목소리 등록 안정성 | 통과 | 오디오 콜백, 연속 입력 시간 오차, 취소·재등록 회귀 테스트 통과 |
| iPhone 설치 | 완료 | 1.5.1 빌드 15 설치 및 기기 버전 조회 확인 |
