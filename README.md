...
현재 버전은 **1.5.0 (빌드 14)**입니다. 참여자 구분 요청의 모델 호환성을 보완하고, Mac·iPhone 녹음 폴더와 폴더 동기화를 추가했습니다. [변경 및 검증 기록](docs/implementation/PARTICIPANTS-AND-FOLDERS.md)
...
- Mac/iOS 1.5.0 빌드 14를 두 기기에 설치하고 실행을 확인했습니다. 실제 iPhone 목소리 등록은 사용자 확인을 기다립니다. 개인 목소리 등록 데이터는 전송하거나 대신 녹음하지 않았습니다. 자세한 내용은 [목소리 등록 검증 기록](docs/implementation/VOICE-ENROLLMENT-GUIDE.md)에 있습니다.

## 검증 및 배포 현황

현재 문서가 반영하는 버전은 **Mac/iOS 1.5.0, 빌드 14**입니다.

| 항목 | 상태 | 결과 |
| --- | --- | --- |
| Mac 전체 테스트 | 통과 | 419개 통과 |
| iOS 시뮬레이터 전체 테스트 | 통과 | 293개 통과 |
| Cloudflare Worker 테스트 | 통과 | 55개 통과 |
| 폴더 화면 | 통과 | Mac·iPhone 한국어 정적 렌더 확인 |
...
| 1.5.0 빌드 14 Mac Release | 완료 | Release 빌드·서명 검증, 백업 후 설치, 실제 폴더 영역 및 실행 확인 |
| 1.5.0 빌드 14 iOS Release | 완료 | Release 빌드·코드서명·로컬 IPA 검증, 실제 iPhone 설치·실행 확인 |
| Cloudflare Worker 배포 | 완료 | 폴더 마이그레이션 0005와 Worker 배포, 기존 데이터 및 폴더 API 인증 조회 200 |
| iPhone 모델 파일 | 완료 | 공개 모델 파일 13개가 1.3.2 업데이트 후 보존됨을 확인 |
| 목소리 등록 안정성 | 통과 | 오디오 콜백, 연속 입력 시간 오차, 취소·재등록 회귀 테스트 통과 |
| iPhone 설치 | 완료 | 1.5.0 빌드 14 설치·실행 확인 |
