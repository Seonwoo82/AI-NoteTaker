...
public sealed class SpeakerWorkerClient(string root, string? executable = null, int knownSpeakerCount = 0)
{
    ...
    public async Task<VoiceEmbedding> EmbedLiveAsync(byte[] wav, CancellationToken token)
    {
        if (wav.Length > 2 * 1024 * 1024) throw new InvalidDataException("실시간 음성 구간이 너무 큽니다.");
        var result = await RunAsync<VoiceEmbedding>("embed-live", "-", new Progress<string>(_ => { }), token, wav);
        if (result.ModelId != SpeakerModels.EmbeddingModelId || !ValidEmbedding(result.Embedding) || !double.IsFinite(result.SampleDuration) ||
            !double.IsFinite(result.SpeechDuration) || result.SampleDuration is < 1 or > 6 || result.SpeechDuration < 1 || result.SpeechDuration > result.SampleDuration)
            throw new InvalidDataException("최근 발화의 목소리를 확인하기 어렵습니다.");
        return result;
    }
    private async Task<T> RunAsync<T>(string command, string audioPath, IProgress<string> progress, CancellationToken token, byte[]? inputAudio = null)
    {
        ...
        { UseShellExecute = false, CreateNoWindow = true, WindowStyle = ProcessWindowStyle.Hidden, RedirectStandardOutput = true, RedirectStandardError = true, RedirectStandardInput = inputAudio is not null };
        foreach (string argument in new[] { command, inputAudio is null ? Path.GetFullPath(audioPath) : "-", Path.GetFullPath(root), output }) start.ArgumentList.Add(argument);
        ...
        while (runtimeDirectory is not null && !File.Exists(Path.Combine(runtimeDirectory.FullName, "dotnet.exe"))) runtimeDirectory = runtimeDirectory.Parent;
        if (runtimeDirectory is not null) { start.Environment["DOTNET_ROOT"] = runtimeDirectory.FullName; start.Environment["DOTNET_ROOT_X64"] = runtimeDirectory.FullName; }
        ...
        try
        {
            ...
            var writing = inputAudio is null ? Task.CompletedTask : WriteAudioAsync();
            await process.WaitForExitAsync(CancellationToken.None); await messages; await writing;
            ...
            async Task WriteAudioAsync()
            {
                try { await process.StandardInput.BaseStream.WriteAsync(inputAudio!, token); await process.StandardInput.BaseStream.FlushAsync(token); }
                catch (IOException) { } // A failing/cancelled worker may close its pipe before consuming the frame.
