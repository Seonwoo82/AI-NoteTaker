...
| Apple UI 컴포넌트·아이콘 | `Components/AppleTheme.xaml`, WindowControls, AppleIcon, WaveformControl; 원본 PNG를 ICO로 묶은 앱 아이콘. 밝은/어두운·작은 창의 WPF 렌더와 실제 EXE 아이콘 확인 | 보안 안내창 아래의 실제 포인터 전달은 미확인 |
...
| 폴더 생성/이름/삭제/이동/펼침/순서·선택 유지 | `RecordingFolderTests`, `SmokeUi` 폴더 흐름, `SmokeSync`; 생성 시 목록 유지·선택 폴더 녹음/가져오기·삽입선 앞/뒤·재시작 보존 | 실제 OLE 마우스 드래그는 보안 안내창이 치워진 뒤 확인 필요 |
...
| Windows 시스템 오디오 파일 공유 | `WindowsAudioShare`, 실제 WinRT StorageFile·긴 경로 수정·사본 해시·정리/purge 검사 | **DataRequested timeout과 실제 공유 창 미검증. 최종 배포 게이트 미통과** |
...
단계 17의 `4922af1` self-contained ZIP을 새 폴더에 풀어 핵심 10개 검사와 같은 EXE의 동기화·웹 공유 2개 검사를 완료했다. `.verification.json`은 `Passed=false`와 남은 시스템 공유/물리 드래그 검증을 명시한다. 최신 upstream 포함과 Apple 소스 무변경도 다시 확인했다. 최종 배포 스크립트의 `-AllFeatures`에는 재생 보호·동기화·웹 공유까지 추가했으며 13번째 시스템 공유 경로가 아직 통과하지 않았다. 최신 문서도 최종 ZIP에 다시 포함해야 한다.

PC 보안 진단 안내창이 계속 열려 있음을 Windows 창 목록에서 재확인했다. 사용자에게 닫아 달라는 요청이 남아 있으며 보안 앱을 직접 조작하지 않는다. 창이 치워지면 생성한 무음 파일만 있는 격리 라이브러리에서 실제 공유 창과 DataRequested를 확인하고, 폴더 안팎 이동·폴더 앞뒤 정렬의 물리 드래그를 확인한다. 공유 창 timeout을 이 안내창 때문이라고 확정하지 않는다. 오류가 계속되면 실제 창 상태를 바탕으로 원인을 수정한 뒤 같은 게이트를 다시 실행한다.
