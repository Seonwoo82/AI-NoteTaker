...
| 최신 녹음 시작·세션 소유권·빈 파일 보호 | 단계 19의 세션별 콜백·시작 보호 소스 대조와 Core 완료/복구 검사. 단계 20 최신 EXE의 파일 기반 정상 WPF 흐름 통과. 단계 21 같은 패키지 DLL의 빈 입력 완료 거부·UI 복귀·재녹음·종료·재시작·원본 보존 검사 통과 | 실제 장치 중단 검증 |
...
| 폴더 생성/이름/삭제/이동/펼침/순서·선택 유지 | `RecordingFolderTests`, `SmokeUi` 폴더 흐름, `SmokeSync`; 생성 시 목록 유지·선택 폴더 녹음/가져오기·삽입선 앞/뒤·재시작 보존. 단계 18 실제 포인터로 폴더 안팎 이동·앞뒤 정렬. 단계 20 최신 EXE에서도 실제 이동·정렬 후 종료/재실행해 UI와 JSON 보존 확인 | 다른 DPI/다중 모니터의 물리 드래그 |
...
| Windows 시스템 오디오 파일 공유 | 실제 WinRT StorageFile·사본 해시·정리/purge 및 요청별 응답·timeout·취소·중복 콜백 검사. 단계 20 새 EXE에서 실패 후 재시도와 올바른 COM 오류 안내 WPF 검사 통과 | **DataRequested timeout. 탐색기 및 독립 텍스트 공유 진단도 창 미표시. 허용받은 공유 호스트 재시작으로 해결되지 않았으며 최종 게이트 미통과** |
...
최신 EXE와 파일 탐색기에서 공유 창이 나타나지 않았다. 사용자의 재허용 후 `ShellExperienceHost`만 재시작하고 설치된 패키지의 정상 활성화 경로로 복구했지만 앱 공유는 계속 timeout이었다. 독립 WPF 앱에서 생성한 텍스트만 공유하는 최소 진단도 활성 UI 스레드의 호출은 반환됐으나 DataRequested가 오지 않았다. 직접 EXE 실행 때의 XAML 오류는 별도 시도의 실패이며 기존 문제의 원인으로 확정하지 않는다. 앱 외부에서의 재현과 앱 구현의 정상 작동 증명을 구분하고 실제 공유 창·DataRequested 성공을 계속 미완료로 남긴다.

Escape로 중단됐던 폴더 검증은 재허용 후 완료했다. 최신 후보에서 실제 포인터로 C를 A 앞으로 이동하고 앱을 정상 종료·재실행해 C/A/B 순서와 녹음의 폴더 A 배정을 UI·JSON으로 확인했다. 진단·테스트 창을 닫았으며 전체 Explorer 재시작·패키지 재등록·설정 변경·재부팅은 하지 않았다. 공유가 정상 작동하는 Windows 환경에서 남은 네이티브 공유·최종 게이트 검증이 필요하다. 상세 증거는 단계 20에 기록했다.

단계 21에서 누락됐던 WPF 빈 입력 실패도 같은 패키지 어셈블리로 검증했다. 실제 입력 장치 대신 0프레임 WAV를 쓰면서 경과 시간 37초를 반환하는 세션으로 완료·종료·재시작 오류 표시와 정상 재녹음의 1초 저장을 확인했다. 독립 검사 도구의 두 시나리오가 통과했고 패키지 DLL 경로·해시가 일치했다. 별도 호스트 검사이므로 EXE 전체 게이트 결과에 합산하지 않는다. 공유 호스트의 등록된 FullTrustApp 경로를 활성화한 뒤에도 단독 네이티브 공유는 timeout이었다. [단계 21](FULL-PORT-STEP21.md).
