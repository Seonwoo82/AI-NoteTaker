...
| F-003 내 발화와 연속 재생 | 구현·PCM 단위·실제 출력 장치 검증 | 내 발화 필터와 선택/전체 내 발화 이어듣기. 선택 프레임만 연결하는 제공자 테스트. 생성 무음으로 실제 출력 장치의 선택 구간 종료·일반 재생 복귀·일시정지 확인. 실제 음성 청취와 등록 UI 오디오 충돌 연동 추가 점검 필요 |
