...
| 전체 Cloudflare 동기화 및 F-010 | 전체 core 실행기 병합 구현, WPF 연결 대기 | 녹음/폴더/삭제 상태와 회의록/분석/수정 이력/프로필/AI 설정 병합, 원본 바이트·원문 버전, 영속 대기/수신함·cursor/재시도·중단 복구·동시 변경을 실제 Worker HTTP/SQLite와 두 라이브러리로 검증. auto/manual/status UI와 실제 앱 작업 수명 연결은 미구현. [단계 9](docs/implementation/FULL-PORT-STEP9.md) |
| 동기화 비밀 제외 | 전체 core 실행기 경로에서 로컬 키·목소리·엔진 설정 보존 검증 | DTO와 실제 JSON 요청 본문에 API 키/토큰/목소리/임베딩 값이 포함되지 않음을 확인. 기기별 키 보유 여부만 전달하며 읽을 수 없으면 unknown. WPF 설정/종료/녹음 작업과의 연결 검증은 후속 단계 |
...
- 모델 선택/예산·정리/보완·목차와 전체 core 동기화 병합을 구현했다. 다음은 자동/수동/연결 테스트/오류·진행·키 보유 안내의 WPF 연결 및 녹음·재생·AI·설정·종료 수명 조정이다. 이후 지속 공유 검증·장시간 처리·의미 품질·최종 배포까지 이어간다. 4B 모델의 구조화 분석 의미 분류 오류도 모델 비교로 검증한다. 상세 시간 전사는 120초×180구간(6시간), 현재 Sherpa 전체 오디오 로더는 4시간 상한이므로 장시간 처리 범위를 함께 맞추고 검증해야 한다. 실제 Worker/SQLite 왕복은 통과했지만 실제 Apple 기기/Cloudflare 배포 검증과 구분한다.
