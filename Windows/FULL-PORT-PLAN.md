...
| 전체 Cloudflare 동기화 및 F-010 | 전체 통신 계약 및 녹음/폴더 실행기 구현, 문서/설정 병합과 UI 대기 | M4A·메타데이터·폴더·삭제 상태의 양방향 병합, 서버별 영속 대기열/재시도, 동시 변경 보호, 파일 적용 중단 복구를 두 라이브러리와 실제 Worker HTTP/SQLite로 검증. 회의록/분석/수정/프로필/AI 설정의 실행기 병합 및 auto/manual/status UI는 미구현. [단계 8](docs/implementation/FULL-PORT-STEP8.md) |
| 동기화 비밀 제외 | 전용 DTO 및 녹음/폴더 실행기에서 로컬 키·목소리 보존 검증 | DTO와 서버는 API 키/토큰/목소리/임베딩 값을 설정 본문에 포함하지 않는다. 기기별 키 보유 여부만 전달. 프로필/설정 병합의 전체 실행 경로와 UI 검증은 후속 단계 |
