...
public partial class MainWindow
{
    ...
    private void LoadCleanup(TranscriptCache? cache)
    {
        ...
        if (notes?.Cleanup is { } cleanup && cache?.Complete == true)
        {
            try
            {
                cleanup.Validate(CleanupSource.Make(string.Join("\n\n", cache.Chunks), meeting?.Transcript, cleanup.SourceKind));
                cleanedTranscriptText = cleanup.MeetingText(meeting?.Transcript);
                if (cleanup.SourceKind == "speakers") cleanedTurnText = cleanup.TextOverrides;
