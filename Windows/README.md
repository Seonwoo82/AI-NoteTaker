...
공유 창을 다시 열면 앱은 녹음 UUID로 서버 상태를 조회합니다. 링크 주소는 생성한 공유 창에서 복사할 수 있고, 다른 실행에서도 활성 상태 확인과 **공유 취소**가 가능합니다. 공유 중에는 새 링크 생성·링크 열기 버튼을 표시하지 않습니다. 다시 공유하려면 공유를 취소한 뒤 **웹 링크 만들기**를 누르세요. **공유 취소**는 공개 링크만 비활성화하며 로컬 녹음 삭제와는 별개입니다. 로컬 녹음을 삭제해도 이미 만든 공개 스냅샷이 자동으로 취소되지는 않으므로 필요하면 웹 공유 창에서 따로 취소하세요.
