...
동기화는 **녹음·폴더·삭제 상태·회의록·분석·수정 이력·프로필·AI 설정의 core 병합 실행기**와 영속 대기열, 재시도, 중단 복구까지 구현·검증했습니다. 회의록은 생성 당시 원문을 보존하며 받은 문서의 바이트와 해시를 유지합니다. 자동/수동 동기화 UI와 실제 앱 작업 수명 연결은 아직 구현 중입니다. 기존 서버에서 Windows 기기 등록을 사용하려면 `0008_windows_devices.sql`까지 적용하고 Worker를 배포해야 합니다. [동기화 검증과 남은 범위](docs/implementation/FULL-PORT-STEP9.md)
