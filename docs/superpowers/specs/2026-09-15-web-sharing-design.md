...
- PUT /v1/shares/<sourceID>, application/json: {"title":"...","markdown":"..."}. Replaces the previous share for this source. HTTP 200: {"url":"https://worker/s/<random-token>","expiresAt":<Unix milliseconds>}. Server generates 32 cryptographically random bytes encoded as base64url and stores both its SHA-256 and a retrievable token in private D1. Exactly 7 days expiry.
- GET /v1/shares/<sourceID>: authenticated HTTP 200 {"active":true,"url":"https://worker/s/<token>","expiresAt":<Unix milliseconds>} or {"active":false}. Active URLs must remain visible and copyable after closing the sheet, restarting the app, or opening another configured device. Do not recreate a share merely to display its URL. Snapshot/private object contents remain excluded from this response.
