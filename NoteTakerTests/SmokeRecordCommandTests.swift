...
private actor SmokeRecordSessionTestDouble: SmokeRecordCaptureSessioning {
    ...
    private var recordedConfigurations: [RecordingConfiguration] = []
    ...
    func configurations() -> [RecordingConfiguration] {
...
struct SmokeRecordCommandTests {
    ...
    @Test("exact mic-and-system smoke command parses typed duration output and mode")
    func exactMicAndSystemCommandParses() throws {
        let parsed = try #require(try SmokeRecordArguments.parse([
            "--smoke-record", "2.5", "/tmp/smoke-output.m4a", "--mode", "micAndSystem"
        ], environment: smokeEnvironment))
    ...
    @Test("smoke command accepts explicit output clock fallback after mode")
    func explicitOutputClockParses() throws {
        let parsed = try #require(try SmokeRecordArguments.parse([
            "--smoke-record", "2.5", "/tmp/smoke-output.m4a",
            "--mode", "micAndSystem",
            "--clock", "output"
        ], environment: smokeEnvironment))
    ...
    func malformedSmokeCommandsFailDeterministically() throws {
        ...
        #expect(throws: SmokeRecordArgumentError.unknownClockSource("display")) {
            try SmokeRecordArguments.parse([
                "--smoke-record", "1", "/tmp/out.m4a", "--mode", "micAndSystem", "--clock", "display"
            ], environment: smokeEnvironment)
        }
        #expect(throws: SmokeRecordArgumentError.missingClockSource) {
            try SmokeRecordArguments.parse([
                "--smoke-record", "1", "/tmp/out.m4a", "--mode", "micAndSystem", "--clock"
            ], environment: smokeEnvironment)
        }
    }
    ...
    func stimulusDecisionIsPureAndModeSpecific() throws {
        ...
        #expect(SmokeRecordStimulusDecision.make(mode: .micAndSystem, outputURL: output, runToken: smokeRunToken) == .externalSpeechWithSelfExclusionProbe(SmokeRecordProbeSidecars(outputURL: output, runToken: smokeRunToken)))
    }

    @Test("smoke capture passes parsed clock source into recording configuration")
    @MainActor
    func smokeCapturePassesParsedClockSourceIntoConfiguration() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("SmokeRecordClockSource-\(UUID().uuidString)", isDirectory: true)
        ...
        let command = SmokeRecordCommand(session: session, terminate: {})

        await command.run(arguments)

        let configuration = try #require(await session.configurations().first)
        #expect(configuration.clockSource == .output)
    }
